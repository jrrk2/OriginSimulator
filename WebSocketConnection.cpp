...
void WebSocketConnection::sendFrame(quint8 opcode, const QByteArray &payload, bool masked) {
    ...
    if (0) qDebug() << "Sending WebSocket frame - Opcode:" << QString("0x%1").arg(opcode, 2, 16, QChar('0'))
...
//         qDebug() << "Not enough data for frame header";
    ...
    if (0) qDebug() << "Frame header - FIN:" << fin << "Opcode:" << QString("0x%1").arg(opcode, 2, 16, QChar('0')) 
